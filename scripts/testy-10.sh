...
get_game_name() {
  local serial="$1"
  local db_file="wiitdb.xml"  # Nombre del archivo de la base de datos local
  ...
  # Extraer el nombre del juego usando xmlstarlet
  local game_name=$(xmlstarlet sel -t -m "//game[id='$serial']" -v 'title' -n "$db_file")
