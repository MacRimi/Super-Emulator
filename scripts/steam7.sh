...
function install_bin_steam() {
    mkdir -p "$md_inst/bin"
    echo "Descargando steam_latest.deb..."
    wget --content-disposition https://steamcdn-a.akamaihd.net/client/installer/steam_latest.deb -O "$md_inst/bin/steam_latest.deb"
    if [ $? -ne 0 ]; then
        echo "Error: Fallo al descargar steam_latest.deb"
        return 1
    fi

    echo "Instalando steam_latest.deb..."
    sudo dpkg -i "$md_inst/bin/steam_latest.deb" || sudo apt -f install -y
    if [ $? -ne 0 ]; then
        echo "Error: Fallo al instalar steam_latest.deb"
        return 1
    fi

    rm "$md_inst/bin/steam_latest.deb"
