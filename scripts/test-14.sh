...
db_file="$script_dir/psxdatacenter.xml"  # Especifica la ruta completa al archivo de la base de datos local
...
for game_dir in "$rpcs3_game_dir"/*; do
  if [ -d "$game_dir" ]; then
    ...
    # Imprimir el serial para depuración
    echo "Procesando serial: $serial"
    ...
    if [ -z "$game_name" ]; then
      echo "No se encontró el nombre del juego para el serial $serial. Usando el serial como nombre."
      game_name="No_game_name_found_for_serial_$serial"
    fi
