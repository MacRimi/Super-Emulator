...
mount_nas() {
    ...
    for ((i=1; i<=ATTEMPTS; i++)); do
        ...
        mount -t cifs -o username=$NAS_USERNAME,password=$NAS_PASSWORD,mfsymlinks,nounix //$NAS_IP/$NAS_SHARE $NAS_MOUNT_POINT
        if [ $? -eq 0 ]; then
...
case $MAIN_OPTION in
    1)
        echo "Creando archivo de señalización para la copia de seguridad en el próximo arranque..."
        touch $FLAG_FILE
        echo "Reiniciando el sistema para realizar la copia de seguridad..."
        reboot
        ;;
    2)
        restore
