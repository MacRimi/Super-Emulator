...
check_volume() {
  local LV_PATH
  LV_PATH=$(lvscan | grep "ACTIVE" | awk '{print $2}' | tr -d "'")
  if [ -z "$LV_PATH" ]; then
  ...
  local FREE_SPACE
  FREE_SPACE=$(vgdisplay | grep "Free  PE / Size" | awk '{print $5}')
...
extend_volume() {
  ...
  local CURRENT_SIZE
  CURRENT_SIZE=$(lvdisplay "$LV_PATH" | grep "Current LE" | awk '{print $3}')
  local MAX_SIZE
  MAX_SIZE=$(vgdisplay | grep "Total PE" | awk '{print $3}')
