...
function pre1() {
    ...
    if [[ ! -s "$ubp" ]] || [[ ! -s "$bp" ]] || [[ ! -s "/userdata/system/pro/steam/prepare.sh" ]]; then
      ...
      mkdir -p "$c/usr/bin" 2>/dev/null
      wget -q --tries=30 --no-check-certificate --no-cache --no-cookies -O "$ubp" "https://raw.githubusercontent.com/MacRimi/Super-RetroPie/main/steam/build/prepare.sh"
      ...
      wget -q --tries=30 --no-check-certificate --no-cache --no-cookies -O "$env" "https://raw.githubusercontent.com/MacRimi/Super-RetroPie/main/steam/build/env.sh"
...
function pre2() {
    ...
    if [[ ! -s "$f" ]] || [[ ! -s "/userdata/system/.local/share/Conty/passwd" ]]; then
      mkdir -p "$c/etc" 2>/dev/null
      wget -q --tries=30 --no-check-certificate --no-cache --no-cookies -O "$f" "https://raw.githubusercontent.com/MacRimi/Super-RetroPie/main/steam/build/passwd.sh"
    ...
    if [[ ! -s "$c/etc/group" ]] || [[ ! -s "/userdata/system/.local/share/Conty/group" ]]; then
      mkdir -p "$c/etc" 2>/dev/null
      wget -q --tries=30 --no-check-certificate --no-cache --no-cookies -O "$f" "https://raw.githubusercontent.com/MacRimi/Super-RetroPie/main/steam/build/group.sh"
      dos2unix "$f" 2>/dev/null
      chmod 777 "$f" 2>/dev/null
...
function pre3() {
    if [[ ! -s "$c/usr/bin/lutris" ]]; then
      mkdir -p "$c/usr/bin/" 2>/dev/null
      mkdir -p "$c/bin/" 2>/dev/null
      wget -q --tries=30 --no-check-certificate --no-cache --no-cookies -O "$c/usr/bin/lutris" "https://raw.githubusercontent.com/MacRimi/Super-RetroPie/main/steam/build/lutris.sh"
